...
const cargosPermitidos = ["Diretoria","Liderança"]
users.forEach((user, x)=>{
      if((offset <= x && limit >= x) && cargosPermitidos.includes(user.cargo)){
            if(user.email.match("[0-9a-zA-Z\.]{1,}@oilcorp.com.br")){
                  console.log(`Nome: ${user.nome}, Cargo: ${user.cargo}, Email: ${user.email}`)
                  personFind = true
            }
      }
})
